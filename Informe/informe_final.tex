...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\linewidth]{pipeline_diagrama.png}
  \caption{Esquema del pipeline. Los modelos operan sobre la imagen cruda. El
  preprocesamiento se estudia como etapa de caracterización de la imagen y queda
  disponible en la interfaz como herramienta de visualización, fuera de la cadena
  de inferencia.}
  \label{fig:pipeline}
\end{figure}
...
Sobre imágenes reales del conjunto, el preprocesamiento canónico (NLM con
...
escala de grises y la preprocesada arroja valores altos de PSNR
(\(\approx\SI{33}{dB}\)) y de índice de similitud estructural
SSIM~\cite{wang2004ssim} (\(\approx0.97\)): el filtrado limpia el fondo sin degradar
el bandeo G diagnóstico. Estos valores confirman que el ruido de partida es muy bajo,
de modo que el margen de mejora del denoising es acotado; junto con el principio de
consistencia entre entrenamiento e inferencia, esto fundamenta la decisión de correr
los modelos sobre la imagen cruda (Sección~\ref{sec:preproc}).
