...
El cariotipado es un procedimiento de rutina en citogenética clínica que consiste en
detectar, clasificar y ordenar los cromosomas de una célula en metafase para
diagnosticar alteraciones cromosómicas. Realizado manualmente es lento (30 a 50
minutos por caso) y depende de personal altamente especializado, lo que motiva su
automatización. En este trabajo se desarrolla un pipeline automático de cariotipado
sobre el dataset público AutoKary2022, integrando las etapas clásicas del
procesamiento de imágenes: caracterización de la imagen y preprocesamiento
(estimación de ruido, denoising Non-Local Means y realce de contraste CLAHE),
segmentación de instancias con Mask R-CNN, extracción de características y
registración de cada cromosoma a una pose canónica mediante PCA, clasificación con
una red convolucional VGG16 con atención Squeeze-and-Excitation, y ensamblado del
cariograma en la grilla estándar con conteo por par y marcado de anomalías. La
segmentación alcanza un AP@0.5 del \SI{97.8}{\percent} sobre el conjunto de test y la
clasificación una exactitud del \SI{87}{\percent} sobre las 24 clases cromosómicas.
Se presenta además una interfaz gráfica en Streamlit que ejecuta y visualiza cada
etapa del procesamiento, corriendo los modelos sin dependencias pesadas (segmentación
en TorchScript y clasificación en ONNX). Los resultados se contrastan con el estado
del arte y se discuten las mejoras pendientes.
...
Una decisión de diseño relevante es que los modelos se ejecutan sobre la imagen
cruda, no sobre una versión preprocesada. El preprocesamiento se aborda como una
etapa de caracterización y análisis de la imagen, cuya conclusión justifica
precisamente esa decisión; los motivos se detallan en la
Sección~\ref{sec:preproc} y se cuantifican en la Sección~\ref{sec:resultados}.

\subsection{Caracterización de la imagen y estudio de preprocesamiento}
\label{sec:preproc}

Antes que un paso obligatorio dentro de la cadena de inferencia, el preprocesamiento
se aborda aquí como una etapa de \textbf{caracterización de la imagen}: se estima el
ruido, se aplican denoising y realce de contraste, y se mide su efecto de forma
cuantitativa. El resultado de este estudio (Sección~\ref{sec:resultados}) es el que
fundamenta la decisión de correr los modelos sobre la imagen cruda. El
preprocesamiento se implementó en el módulo \texttt{preprocessing.py} y consta de
tres pasos aplicados en orden: conversión a escala de grises, denoising y realce
de contraste local.
...
\paragraph{Decisión: los modelos operan sobre la imagen cruda.} Tanto el modelo de
segmentación como el de clasificación se entrenaron sobre imágenes crudas. El
principio rector es que el preprocesamiento debe ser consistente entre entrenamiento
e inferencia: aplicar en inferencia una transformación ausente en el entrenamiento
introduciría un desfasaje entre entrenamiento y test que degradaría el desempeño.
Por eso, en producción ambos modelos reciben la imagen cruda. Esta decisión se apoya,
además, en la caracterización de la imagen (Sección~\ref{sec:resultados}): las
imágenes de AutoKary2022 son microscopía Giemsa profesional con muy poco ruido, de
modo que el denoising aporta poco y un realce agresivo puede borrar el bandeo G,
justamente la información que necesita el clasificador. El preprocesamiento conserva,
no obstante, un rol real: en la interfaz queda disponible como herramienta de
\textbf{visualización} (comparar original y realzada, observar el ruido estimado
antes y después) y como recurso opcional para imágenes claramente más degradadas que
...
Los recortes se generan sobre la imagen cruda (misma entrada con la que se entrenó
el clasificador) y tomando los puntos desde la máscara predicha, de modo que la
salida de esta etapa quede estandarizada para las etapas posteriores del pipeline.
...
el método de Immerkær es muy bajo ya en la imagen original y disminuye con el
filtrado (\(\sigma \approx 0.7 \rightarrow 0.2\) en promedio), lo que indica que las
imágenes parten prácticamente limpias y el margen de mejora del denoising es
acotado. El índice de similitud estructural SSIM~\cite{wang2004ssim} entre la imagen
original en escala de grises y la preprocesada es alto (\(\approx0.95\)),
confirmando que el realce preserva la estructura del bandeo G diagnóstico; el PSNR
moderado (\(\approx\SI{21}{dB}\)) refleja, en cambio, el cambio de contraste que
introduce CLAHE de forma deliberada. En conjunto, estos valores muestran que
preprocesar aporta poco a la calidad de la imagen; junto con el principio de
consistencia entre entrenamiento e inferencia, fundamentan la decisión de correr los
modelos sobre la imagen cruda (Sección~\ref{sec:preproc}).
