...
de validación). El análisis por clase (Tabla~\ref{tab:clasif-report},
Anexo~\ref{anexo:clasif}) revela que el error residual se concentra en un grupo de
cromosomas medianos y pequeños de morfología similar (clases 7, 8, 9, 10, 12, 13 y
14), la parte históricamente más difícil del cariotipado; la clase minoritaria
(clase 16, con solo 24 muestras de test) es la de menor desempeño (F1 = 0.69) pese
al balanceo por pesos.
...
\begin{table}[H]
  \centering
  \small
  \caption{Reporte de clasificación por clase del modelo final (VGG16 + SE +
  \texttt{class\_weight} + \textit{label smoothing} + \(L_2\)).}
